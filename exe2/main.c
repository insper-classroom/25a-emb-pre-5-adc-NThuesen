...
void led_task_G(void *p) {
    gpio_set_function(PWM_0_PIN_G, GPIO_FUNC_PWM);
    uint slice_num = pwm_gpio_to_slice_num(PWM_0_PIN_G);
    pwm_set_clkdiv(slice_num, 125);
    pwm_set_wrap(slice_num, 100);
...
int main() {
    ...
    xTaskCreate(led_task_G, "LED_Task G", 256, NULL, 1, NULL);
